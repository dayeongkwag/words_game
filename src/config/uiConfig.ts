...
export const UI_CONFIG = {
  ...
  input: {
    /**
     * 마지막으로 친 음절을 스스로 확정하기까지 기다리는 시간(ms).
     *
     * 한글은 다음 글자를 쳐야 앞 음절이 확정된다("가" → "각" 이 될 수 있으므로).
     * 낱말의 끝 글자를 치고 멈추면 확정이 안 되어 채점이 일어나지 않으므로,
     * 입력이 멎으면 자동으로 확정한다.
     * 받침을 덧붙이는 데 걸리는 시간(보통 0.3초 이내)보다 넉넉해야 한다.
     */
    idleCommitMs: 1200,
  },
